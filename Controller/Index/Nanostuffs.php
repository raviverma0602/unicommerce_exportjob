...
class Nanostuffs extends \Magento\Framework\App\Action\Action
{
  ...
  public function execute()
  {
    ...
    $writer = new \Zend_Log_Writer_Stream(BP . '/var/log/uniwareexecuteCode_Nanostuffs.log');
    $logger = new \Zend_Log();
    $logger->addWriter($writer);
    $logger->info('Execute Code'); //To print simple text log
    $logger->info(print_r("Status Job Code -> ".$this->getStatusJob(), true));
  ...
  public function getBearerToken()
  {
    ...
    $writer = new \Zend_Log_Writer_Stream(BP . '/var/log/uniwareBearerToken_Nanostuffs.log');
    $logger = new \Zend_Log();
    $logger->addWriter($writer);
    $logger->info('token'); 
  ...
  public function getCreateJob()
  {
    ...
    $writer = new \Zend_Log_Writer_Stream(BP . '/var/log/uniwareBearerTokenCheck_Nanostuffs.log');
    $logger = new \Zend_Log();
    $logger->addWriter($writer);
    ...
    $result = curl_exec($curl);
    $data = json_decode($result);
    $writer = new \Zend_Log_Writer_Stream(BP . '/var/log/uniwarejobCreateCheck_Nanostuffs.log');
    $logger = new \Zend_Log();
    $logger->addWriter($writer);
  ...
  public function getStatusJob()
  {
    ...
    $writer = new \Zend_Log_Writer_Stream(BP . '/var/log/uniwarejobCreateLogCheck_Nanostuffs.log');
    $logger = new \Zend_Log();
    $logger->addWriter($writer);
    ...
    $writer = new \Zend_Log_Writer_Stream(BP . '/var/log/uniwaregetStatusJobCheck_Nanostuffs.log');
    $logger = new \Zend_Log();
    $logger->addWriter($writer);
  ...
  {   $date = date('Y-m-d');
      $writer = new \Zend_Log_Writer_Stream(BP . '/var/log/unicommerceorderstatus/unicommerce_status_nanostuffsapi'.$date.'.log');
      $logger = new \Zend_Log();
      $logger->addWriter($writer);
      $logger->info($msg);
  }
