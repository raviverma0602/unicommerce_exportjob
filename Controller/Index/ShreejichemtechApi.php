...
class ShreejichemtechApi extends \Magento\Framework\App\Action\Action
{
  ...
  public function execute()
  {
    ...
    $writer = new \Zend_Log_Writer_Stream(BP . '/var/log/uniwareexecuteCode_Shreeji.log');
    $logger = new \Zend_Log();
  ...
  public function getBearerToken()
  {
    ...
    $data = json_decode($result);
     $writer = new \Zend_Log_Writer_Stream(BP . '/var/log/uniwareBearerToken_Shreeji.log');
    $logger = new \Zend_Log();
    $logger->addWriter($writer);
    $logger->info('token'); 
  ...
  public function getCreateJob()
  {
    ...
    $writer = new \Zend_Log_Writer_Stream(BP . '/var/log/uniwareBearerTokenCheck_Shreeji.log');
    $logger = new \Zend_Log();
    $logger->addWriter($writer);
    ...
    $result = curl_exec($curl);
    $data = json_decode($result);
    $writer = new \Zend_Log_Writer_Stream(BP . '/var/log/uniwarejobCreateCheck_Shreeji.log');
    $logger = new \Zend_Log();
    $logger->addWriter($writer);
  ...
  public function getStatusJob()
  {
    ...
    $writer = new \Zend_Log_Writer_Stream(BP . '/var/log/uniwarejobCreateLogCheck_Shreeji.log');
    $logger = new \Zend_Log();
    $logger->addWriter($writer);
    $logger->info('jobCreate'); 
    ...
    $writer = new \Zend_Log_Writer_Stream(BP . '/var/log/uniwaregetStatusJobCheck_Shreeji.log');
    $logger = new \Zend_Log();
    $logger->addWriter($writer);
  ...
  {   $date = date('Y-m-d');
      $writer = new \Zend_Log_Writer_Stream(BP . '/var/log/unicommerceorderstatus/unicommerce_status_shreejichemtech'.$date.'.log');
      $logger = new \Zend_Log();
      $logger->addWriter($writer);
      $logger->info($msg);
  }
