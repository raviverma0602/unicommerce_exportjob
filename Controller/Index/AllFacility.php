...
class AllFacility extends \Magento\Framework\App\Action\Action
{
  ...
  public function __construct(
    ...
    \Unicommerce\ExportJob\Model\ResourceModel\Order\CollectionFactory $orderCollectionFactory
  ...
  {
    $writer = new \Zend_Log_Writer_Stream(BP . '/var/log/uniwareexecuteCode_visionlog.log');
    $logger = new \Zend_Log();
    ...
    foreach ($categories as $category) 
    {
      ...
      $saleOrderItemCode = $category['Sale Order Item Code'];
      $orderCollection = $this->orderCollectionFactory->create()->addFieldToFilter('sale_order_item_code', $saleOrderItemCode);
  ...
  public function getBearerToken()
  {
    ...
    $data = json_decode($result);
     $writer = new \Zend_Log_Writer_Stream(BP . '/var/log/uniwareBearerToken_visionlog.log');
    $logger = new \Zend_Log();
    $logger->addWriter($writer);
    $logger->info('token'); 
  ...
  public function getCreateJob()
  {
    ...
    $writer = new \Zend_Log_Writer_Stream(BP . '/var/log/uniwareBearerTokenCheck_visionlog.log');
    $logger = new \Zend_Log();
    $logger->addWriter($writer);
    ...
    $result = curl_exec($curl);
    $data = json_decode($result);
    $writer = new \Zend_Log_Writer_Stream(BP . '/var/log/uniwarejobCreateCheck_visionlog.log');
    $logger = new \Zend_Log();
    $logger->addWriter($writer);
  ...
  public function getStatusJob()
  {
    ...
    $writer = new \Zend_Log_Writer_Stream(BP . '/var/log/uniwarejobCreateLogCheck_visionlog.log');
    $logger = new \Zend_Log();
    $logger->addWriter($writer);
    $logger->info('jobCreate'); 
    ...
    $writer = new \Zend_Log_Writer_Stream(BP . '/var/log/uniwaregetStatusJobCheck_visionlog.log');
    $logger = new \Zend_Log();
    $logger->addWriter($writer);
    ...
    // Local path where the file should be saved with the current date in the filename
    // $saveTo = __DIR__ . "/pub/csv_files/file_" . $currentDate . ".csv";
    $saveTo = BP . "/pub/csv_files/" . $currentDate . "/unicommerce_file_".$currentDateTime.".csv";
    ...
    if ($csvContent !== false) {
      ...
      $result = file_put_contents($saveTo, $csvContent);

      if ($result !== false) {
        echo "File has been saved successfully to $saveTo";
      } else {
  ...
  {   $date = date('Y-m-d');
      $writer = new \Zend_Log_Writer_Stream(BP . '/var/log/unicommerceorderstatus/unicommerce_status_vision'.$date.'.log');
      $logger = new \Zend_Log();
      $logger->addWriter($writer);
      $logger->info($msg);
  }
